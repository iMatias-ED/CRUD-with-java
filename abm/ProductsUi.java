...
public class ProductsUi extends JFrame implements ActionListener{
    ...
    private JLabel lb_id, lb_product;
    private JTextField tx_id, tx_product;
    ...
    public ProductsUi(){
        ...
        InputsPanel();
        TablePanel();
        setTableData(db.getAllProducts());
        //window config
        setLayout(null);
        setResizable(false);
    ...
    public void actionPerformed(ActionEvent e){
        if (e.getSource().equals(bt_save)){
            ...
            if (tx_id.getText().equals("")){
                ...
                db.modifyProduct(productData);
            }
        }
        if (e.getSource().equals(bt_edit)){
            setFieldsData(db.getProductById(table.getValueAt(table.getSelectedRow(), 0)));
        }
        if (e.getSource().equals(bt_delete)){
            if(JOptionPane.showConfirmDialog(this, "Esta seguro de eliminar este producto?")==0){
                db.deleteProduct(table.getValueAt(table.getSelectedRow(), 0));
            }
        }
    ...
    private void InputsPanel(){
        ...
        tx_product = new JTextField();
        //labels
        lb_id = new JLabel("ID");
        lb_product = new JLabel("PRODUCT");
        //creating Panel
        tx_panel = new JPanel();
        tx_panel.setLayout(null);
        tx_panel.setBackground(Color.MAGENTA);
        tx_panel.setBounds(0, 0, 800, 200);
        tx_panel.setVisible(true);

        //add inputs 
        tx_panel.add(tx_id);
        tx_panel.add(tx_product);
        //add labels
        tx_panel.add(lb_id);
        tx_panel.add(lb_product);

        //set position and size to labels
        lb_id.setBounds(10, 40, 100, 40);
        lb_product.setBounds(10, 130, 100, 40);
        //set position and size inputs
        tx_id.setBounds(110, 40, 670, 40);
        tx_product.setBounds(110, 130, 670, 40);
    }
    ...
    private void setFieldsData(String [] product){
        tx_id.setText(product[0]);
        tx_product.setText(product[1]);
    }
